...
NeedAnswerOne_fake_broker_and_worker(
    ...
    B_CHECK_EX(b_signal_await(respond_signal));
...
TEST(TestClient, NeedAnswerOne) {
...

    {
        ...
        B_Signal respond_signal;
        B_CHECK_EX(b_signal_init(&respond_signal));
        B_CHECK_EX(b_signal_raise(&respond_signal));
        ...
        B_CHECK_EX(b_fiber_context_fork(
            ...
            [&]() {
                ...
                B_Exception *ex = b_client_need_answer_one(
                    ...
                    &answer);
                fork_finished = true;
                // TODO(strager): Exception should be an
                // precondition exception.
                EXPECT_NE(nullptr, ex);
